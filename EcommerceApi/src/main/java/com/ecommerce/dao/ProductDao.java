...
//            connection = jdbcTemplate.getDataSource().getConnection();
            ...
            while (resultSet.next()) {
                ...
                Optional<Category> categoryopt = categoryList.stream().filter(e->  e.getId() == categoryid).findFirst();
                Category category = categoryopt.isPresent() ? categoryopt.get() : null;
                int accoutid = resultSet.getInt(7);

               Optional<Account> optacc=  accounts.stream().filter(e-> e.getId() ==accoutid).findFirst();
               Account account = optacc.isPresent() ? optacc.get() : null;


                        //accountDao.getAccount(resultSet.getInt(7));
